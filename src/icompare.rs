...
impl<T: Comparer> IComparer<T> {
    ...
    pub fn u_compare(&self, a: &[u8], b: &[u8]) -> Ordering {
    ...
    pub fn compare(&self, a: &[u8], b: &[u8]) -> Ordering {
        let a = internalKey::new(a);
        let b = internalKey::new(b);
        let x = self.u_compare(a.ukey(), b.ukey());
        if x == Ordering::Equal {
            let m = a.num();
            let n = b.num();
            if m > n {
